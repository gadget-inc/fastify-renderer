...
export const useTransitionLocation = ({ base = '' } = {}) => {
    ...
    useEffect(() => {
        ...
        const checkForUpdates = () => {
            const pathname = currentPathname(base), destination = pathname + location.search + location.hash;
...
const currentPathname = (base, path = location.pathname + location.hash) => !path.toLowerCase().indexOf(base.toLowerCase()) ? path.slice(base.length) || '/' : '~' + path;
