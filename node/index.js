    ...
    for (const [index, { fastify, routes, plugin, vite }] of Object.entries(instances)) {
        ...
        await vite_1.build({
            ...
            build: {
                ...
                // this is strange -- usually the manifest is for the server to look at to figure out client paths. Since we use virtual entrypoints for the server as well, we need to manifest them as well so we can figure out the right path for the built virtual entrypoint in production
                manifest: true,
                ssr: true,
            },
        });
