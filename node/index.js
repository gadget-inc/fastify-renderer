...
const FastifyRenderer = fastify_plugin_1.default(async (fastify, incomingOptions) => {
    ...
    fastify.addHook('onRoute', function (routeOptions) {
        if (routeOptions.render) {
            const oldHandler = tracing_1.wrap('fastify-renderer.getProps', routeOptions.handler);
            ...
            routeOptions.handler = tracing_1.wrap('fastify-renderer.handler', async function (request, reply) {
    ...
    fastify.addHook('onReady', async () => {
        ...
        if (plugin.devMode) {
            ...
            devServer = await vite_1.createServer(fastify[symbols_1.kRendererViteOptions]);
            viteMountInstance.use(devServer.middlewares);
            config = devServer.config;
        }
        else {
            config = await vite_1.resolveConfig(fastify[symbols_1.kRendererViteOptions], 'serve');
        }
