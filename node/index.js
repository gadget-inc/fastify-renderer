...
const build = async () => {
    ...
    for (const [index, { fastify, routes, plugin, vite }] of Object.entries(instances)) {
        ...
        await fs_1.promises.writeFile(path_1.default.join(plugin.outDir, 'server', 'virtual-manifest.json'), JSON.stringify(Object.fromEntries(Object.entries(serverEntrypoints).map(([key, value]) => [value, key]))));
    }
};
exports.build = build;
