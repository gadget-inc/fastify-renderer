...
const plugin = async (fastify, incomingOptions) => {
    ...
    fastify.decorate('registerRenderable', function (renderable) {
        const renderableRoute = { ...this[symbols_1.kRenderOptions], renderable };
        return plugin.register(renderableRoute);
    });
    const render = async (reply, renderableRoute, props) => {
        if (reply.sent)
            return;
        ...
        switch (reply.request.accepts().type(['html', 'json'])) {
            ...
            case 'html':
                ...
                const render = { ...renderableRoute, request: reply.request, reply, props };
    ...
    fastify.decorateReply('render', async function (token, props) {
        ...
        const renderableRoute = {
            ...this.server[symbols_1.kRenderOptions],
    ...
    await node_fs_1.promises.mkdir(plugin.clientOutDir, { recursive: true });
...
const FastifyRenderer = (0, fastify_plugin_1.default)(plugin, {
    fastify: '4.x',
    name: 'fastify-renderer',
    dependencies: ['@fastify/accepts', '@fastify/middie'],
});
...
const build = async (fastify) => {
    ...
    await node_fs_1.promises.writeFile(path_1.default.join(plugin.serverOutDir, 'virtual-manifest.json'), JSON.stringify(virtualModulesToRenderedEntrypoints, null, 2));
};
exports.build = build;
Object.assign(FastifyRenderer, { build: exports.build });
