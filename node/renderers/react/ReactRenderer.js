...
class ReactRenderer {
    ...
    async render(render) {
        ...
        try {
            ...
            const { Router } = await this.loadModule('fastify-renderer/client/react');
            ...
            if (this.options.mode == 'streaming') {
                await render.reply.send(this.renderStreamingTemplate(app, bus, server_1.default, render));
            }
            else {
                await render.reply.send(this.renderSynchronousTemplate(app, bus, server_1.default, render));
            }
        }
        catch (error) {
    ...
    buildClientEntrypointModuleURL(id) {
    ...
    buildServerEntrypointModuleURL(id) {
    ...
    startRenderBus(render) {
        ...
        if (this.plugin.devMode) {
            bus.push('tail', `<script type="module" src="${path_1.default.join(this.plugin.assetsHost, this.plugin.base, this.clientEntrypointModuleURL(render.renderable))}"></script>`);
        }
        else {
