...
class ReactRenderer {
    ...
    async render(render) {
        ...
        try {
            ...
            const { React, ReactDOMServer, Router, RenderBusContext, Layout, Entrypoint } = (await this.loadModule(this.entrypointRequirePathForServer(render.renderable))).default;
            let app = (React.createElement(RenderBusContext.Provider, { value: bus },
                React.createElement(Router, { base: this.plugin.base, hook: static_location_1.default(this.stripBasePath(render.request.url)) },
    ...
    buildVirtualServerEntrypointModuleURL(id) {
        return path_1.default.join(SERVER_ENTRYPOINT_PREFIX, id, 'ssr.jsx');
    }
    /**
    ...
    entrypointScriptTagSrcForClient(id) {
        ...
        if (this.plugin.devMode) {
            ...
            if (!manifestEntry) {
                throw new Error(`Module id ${id} was not found in the built assets manifest. Looked for it at ${manifestEntryName} in manifest.json. Was it included in the build?`);
            }
     ...
     * In production
     */
    entrypointRequirePathForServer(id) {
        ...
        if (this.plugin.devMode) {
            ...
            const manifestEntryName = node_1.normalizePath(path_1.default.relative(this.viteConfig.root, entrypointName));
            const manifestEntry = this.plugin.serverEntrypointManifest[manifestEntryName];
            if (!manifestEntry) {
                throw new Error(`Module id ${id} was not found in the built server entrypoints manifest. Looked for it at ${manifestEntryName} in virtual-manifest.json. Was it included in the build?`);
            }
