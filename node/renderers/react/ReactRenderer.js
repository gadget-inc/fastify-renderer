...
class ReactRenderer {
    constructor(plugin, options) {
        ...
        this.basePathRegexp = new RegExp(`^${utils_1.escapeRegex(this.plugin.viteBase)}`);
    ...
    async render(render) {
        ...
        try {
            ...
            let app = (React.createElement(RenderBusContext.Provider, { value: bus },
                React.createElement(Router, { base: render.base, hook: staticLocationHook(this.stripBasePath(render.request.url)) },
    ...
    buildVirtualClientEntrypointModuleURL(route) {
        return (path_1.default.join(this.plugin.viteBase, CLIENT_ENTRYPOINT_PREFIX, route.renderable, 'hydrate.jsx') +
            '?' +
    ...
    buildVirtualServerEntrypointModuleURL(route) {
     ...
     * In production
     */
    entrypointRequirePathForServer(render) {
        ...
        if (this.plugin.devMode) {
            ...
            if (!manifestEntry) {
                throw new Error(`Module id ${render.renderable} was not found in the built server entrypoints manifest. Looked for it at ${entrypointName} in virtual-manifest.json. Was it included in the build?`);
            }
    ...
    startRenderBus(render) {
        ...
        if (this.plugin.devMode) {
            bus.push('tail', `<script type="module" src="${path_1.default.join(this.plugin.assetsHost, render.base, this.entrypointScriptTagSrcForClient(render))}"></script>`);
        }
        else {
            const manifestEntryName = node_1.normalizePath(path_1.default.relative(this.viteConfig.root, entrypointName));
